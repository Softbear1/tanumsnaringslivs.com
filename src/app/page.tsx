  ...
  try {
    ...
    if (catsResult.data && catsResult.data.length > 0) {
      categories = catsResult.data.map((c) => ({
        ...
        bgColor: c.bg_color, sortOrder: c.sort_order,
      }));
    }
    if (bizResult.data && bizResult.data.length > 0) {
      businesses = bizResult.data.map((b) => ({
        id: b.id, name: b.name, categoryId: b.category_id,
        description: b.description, phone: b.phone, email: b.email,
        website: b.website ?? undefined, address: b.address,
        initials: b.initials, boosted: b.boosted, featured: b.featured,
        rating: Number(b.rating), reviewCount: b.review_count,
      }));
    }
  } catch {
    // static fallback
  ...
  return (
    <>
      ...
      <main className="flex-1">
        <DirectoryClient categories={categories} businesses={businesses} />
        <RegisterCTA />
      </main>
      <Footer />
    </>
