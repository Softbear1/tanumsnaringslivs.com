...
import { ArrowLeft, Sparkles, Loader2, Mail, Phone, Calendar, Users } from "lucide-react";
...
export default function EditJobPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!isNew) {
        const { data: job } = await supabase.from("jobs").select("*").eq("id", id).eq("owner_id", user.id).single();
        ...
        const { data: apps } = await supabase.from("applications").select("*").eq("job_id", id).order("created_at", { ascending: false });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isNew) {
      ...
      const { error: err } = await supabase.from("jobs").update(payload).eq("id", id).eq("owner_id", userId);
      if (err) { setError(err.message); setSaving(false); return; }
    }
